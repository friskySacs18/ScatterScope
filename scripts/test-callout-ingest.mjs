...
const db={prepare(sql){return{bind(...args){return{
  ...
  async all(){return{results:[...rows.values()].map(x=>({id:x.id,caller:x.caller,mint:x.mint,publishedAt:x.publishedAt,observedAt:x.observedAt}))}},
  async run(){
    if(sql.startsWith('INSERT OR IGNORE')){const [id,caller,mint,publishedAt,observedAt]=args;if(rows.has(id))return{meta:{changes:0}};rows.set(id,{id,caller,mint,publishedAt,observedAt});return{meta:{changes:1}}}
...
const now=Date.now(),call={type:'callout',id:'callout.event-123',caller,mint,publishedAt:now-5000,observedAt:now};
...
assert.deepEqual((await response.json()).callouts,[{id:call.id,caller,mint,publishedAt:call.publishedAt,observedAt:now}]);
