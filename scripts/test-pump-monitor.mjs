...
const db={prepare(sql){return {bind(...args){return {
  ...
  async run(){if(sql.startsWith('INSERT OR IGNORE INTO monitored_callouts'))observations.set(args[0],args);if(sql.startsWith('INSERT INTO caller_mint_history')){const earlier=history.get(args[0]);if(!earlier||args[4]<earlier[4])history.set(args[0],args)}if(sql.startsWith('INSERT INTO callout_ingest_state'))states.set(args[0],args);return {meta:{changes:1}}}
}},async all(){return {results:[{callers:JSON.stringify([{wallet:caller}])}]}}}},async batch(statements){for(const statement of statements)await statement.run()}};
...
try{
  ...
  const success=await call();assert.equal(success.status,200);assert.equal((await success.json()).executionEnabled,false);
  ...
  assert.equal((await call()).status,200);assert.equal(observations.size,2);
