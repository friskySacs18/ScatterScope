...
function AutoTradeSetup({address}){
  ...
  const [spend,setSpend]=useState(String(saved.spend??.5));
  const [first,setFirst]=useState(String(saved.profit1Percent??50));
  const [firstSell,setFirstSell]=useState(String(saved.profit1Sell??50));
  const [second,setSecond]=useState(String(saved.profit2Percent??100));
